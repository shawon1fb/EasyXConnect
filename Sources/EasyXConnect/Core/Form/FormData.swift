...
public struct FormData {
  ...
  private static let boundaryCharacters = Array(
    "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789")

  public static func generateBoundary() -> String {
    let randomBoundary = String(
      (0..<maxBoundaryLength).map { _ in boundaryCharacters.randomElement()! })
  ...
  public func toBytes() -> Data? {
    ...
    for file in files {
      data.append(boundaryPrefix.data(using: .utf8)!)
      data.append(
        "Content-Disposition: form-data; name=\"\(file.0)\"; filename=\"\(file.1.filename)\"\r\n"
          .data(using: .utf8)!)
